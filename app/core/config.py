...
class Settings(BaseSettings):
    ...
    SUPABASE_SERVICE_ROLE_KEY: Optional[str] = (
        None  # Only for storage operations
    )
    ...
    def clean_supabase_config(self):
        ...
        if self.SUPABASE_SERVICE_ROLE_KEY:
            self.SUPABASE_SERVICE_ROLE_KEY = (
                self.SUPABASE_SERVICE_ROLE_KEY.strip()
            )
        return self
    ...
    # OCR Configuration
    OCR_SERVICE: str = (
        "tesseract"  # Options: tesseract, google_vision, aws_textract
    )
    ...
    def assemble_cors_origins(cls, v: dict[str, Any]) -> dict[str, Any]:
        if isinstance(v.get("BACKEND_CORS_ORIGINS"), str) and not v.get(
            "BACKEND_CORS_ORIGINS", ""
        ).startswith("["):
            v["BACKEND_CORS_ORIGINS"] = [
                i.strip() for i in v["BACKEND_CORS_ORIGINS"].split(",")
            ]
    ...
    def CORS_ORIGINS(self) -> list[str]:
        ...
        defaults = [
            ...
            "http://127.0.0.1:3000",
            "http://127.0.0.1:5173",
            "https://mediguidemvp.netlify.app",
        ]
